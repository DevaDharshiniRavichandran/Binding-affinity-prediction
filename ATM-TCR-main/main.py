...
def load_data_split_random(x_pep, x_tcr, y, test_size=0.2, random_seed=1039):
    """
    Split the dataset into train and test sets using random sampling.
    """
...
def main():
    ...
    # With the random split function
    idx_train, idx_test, idx_test_remove = load_data_split_random(x_pep, x_tcr, y, test_size=0.2, random_seed=args.seed)
    ...
    if args.mode == 'train':
        ...
        if args.save_model:
            ...
            torch.save(model.state_dict(), f'./models/modified.ckpt')
            # torch.save(model.state_dict(), model_name)
    elif args.mode == 'test':

        # Specify the epoch number of the model checkpoint you want to load
        checkpoint_epoch = 30  # Change this to the desired epoch number
        model_name = 'modified.ckpt'

        # Ensure the specified checkpoint file exists in the './models' directory
        assert model_name in os.listdir('./models'), f"Checkpoint {model_name} not found in ./models"

        model_name = './models/' + model_name
        model.load_state_dict(torch.load(model_name, map_location=torch.device('cpu')))

        print(f'[TEST] Loaded checkpoint: {model_name}')
